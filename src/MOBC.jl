...
export solve_BC, parseGAP, parseGAP_orlib
...
function solve_BC(vm, limit=500 ;  showplot = false, docovercuts = true , global_branch = false, use_nsga = true, global_nsga = true, lift_covers = false)
	...
	if !issorted(YN_convex, by = x->x[1])
		s = sortperm(YN_convex, by = e -> e[1])
	...
	for i = 1:length(YN_convex)-1
		...
		while !isempty(S) && cpt < limit
			...
			process_node(pop!(S), S, sense, LN, Ƶ1, Ƶ2, LNGlobal, cstrData, showplot, docovercuts, lift_covers)
			nbNodesTotal += 1
			cpt += 1
		end
...
function process_node(n::Node, S, sense, LN, obj1, obj2, LNGlobal, cstrData, showplot, docovercuts, lift_covers)
	...
	if isbinary(n.x)
		...
		if isweaklydominated(z1, z2, LN)
			...
			if docovercuts && n.nbcover <= 10 && find_cover_cuts(n, cstrData, lift_covers)
				n.nbcover += 1
				process_node(n, S, sense, LN, obj1, obj2, LNGlobal, cstrData, showplot, docovercuts, lift_covers)
			else
				n1, n2 = basicbranch(n)
				push!(S, n1)
				push!(S, n2)
...
function paretobranch(sense::Type{Min}, n, z1, z2, LN, obj1, obj2, showplot)
	...
	showplot && plot_pareto_branch(LN, z1, z2, boundz1, boundz2, sleeptime=2)
	#@assert length(res) >= 1
	res
end
